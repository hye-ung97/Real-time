<?php
	...
	$foodName = $_POST["foodlist"];

	$foodName = preg_replace("/[#\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $foodName);
...
<html>
	<head>
		...
		function drawChart() {
			...
			data.addRows([
				<?php
					$sql = 'select DISTINCT userId ,COUNT(DISTINCT userId) as cnt, contents, time from insta WHERE contents="'.preg_replace("/\s+/", "", $foodName).'" group by time, contents order by time';
					...
					<div>
						<div style="display: block; float: left; width: 13%;">
							...
							<div style="margin-top: 30%;">
								<?php
									$sql = 'select url from image where contents="'.$foodName.'"limit 1';
									$result = mysqli_query($conn, $sql);
									while($row = mysqli_fetch_array($result)){
						...
						<div class="hashblock">
							<?php
								$sql = 'select contents,time from daum where cat="음식" and contents="'.$foodName.'" group by contents';
								$result = mysqli_query($conn, $sql);
							...
							<div style="margin-bottom: 1%;"> 총 게시물수 : 
								<?php
									$sql = 'SELECT * FROM instaT WHERE contents = "'.preg_replace("/\s+/", "", $foodName).'" ORDER by time DESC LIMIT 1';
									$result = mysqli_query($conn, $sql);
									while($row = mysqli_fetch_array($result)){
							...
							<div style="margin-bottom: 1%;">
								<?php 
									$sql = 'SELECT hashtag , COUNT(hashtag) as cnt, contents FROM insta WHERE contents ="'.preg_replace("/\s+/", "", $foodName).'"  GROUP BY hashtag ORDER BY cnt DESC limit 5';
									$result = mysqli_query($conn, $sql);
									while($row = mysqli_fetch_array($result)){
							...
							<div style="background-image:url('images/<?php echo $foodName; ?>.PNG'); background-repeat:no-repeat; background-position: center; background-size: contain; height: 175px; margin-top: 2%; width: 315px;"></div>
						</div>
							...
							<section class="demowrap">
								<section class="demo">
									<div id="eclipse1">
										...
										<?php 
												$sql = 'select distinct name, address, total from blogstore where contents = "'.$foodName.'" limit 5';
												$result = mysqli_query($conn, $sql);
												while($row = mysqli_fetch_array($result)){
													echo '<div>';//start
													$name = $row["name"];
