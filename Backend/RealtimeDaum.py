...
def jobDaum():
    reqD = requests.get("https://www.daum.net")
    reqD_text = reqD.text
    soup = BeautifulSoup(reqD_text, "lxml")

    listD = soup.select(".realtime_part > ol >li")

    timeD = datetime.datetime.now()

    for i in listD:
        rank = i.find("span", {"class": "ir_wa"}).text
        word = i.find("a", {"class": "link_issue"}).text
        ...
        sql = 'insert into daum value (' + rank.replace("위","") + ',"' + word + '","' + str(timeD)[:16] + '","' + cat + '","'+color+'")'
        cur.execute(sql)
        conn.commit()


schedule.every(10).minutes.do(jobDaum)
